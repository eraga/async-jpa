...
internal fun EntityManagerFactory.closeAndDeleteDBFiles(persistenceUnit: String) {
    this.close()
    if(this !is EntityManagerFactoryImpl)
        return

    val puMetadata = KunderaMetadataManager
            .getPersistenceUnitMetadata(
                    this.kunderaMetadataInstance,
                    persistenceUnit
            )


    puMetadata
            .getProperty(PersistenceProperties.KUNDERA_DATASTORE_FILE_PATH)
            ?.apply {
                FileUtils.deleteRecursively(File(this))
            }
}
